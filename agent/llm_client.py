...
class LLMClient:
    ...
    def _parse_response_from_text(self, text: str, tools: list = None) -> 'LLMResponse':
        """从纯文本（流式累积结果）解析 tool call 或文本响应。"""
        result = LLMResponse()
        result.text = text

        parsed = self._try_parse_tool_call_from_text(text)
        if parsed:
            result.has_tool_call = True
            result.tool_name = parsed["name"]
            result.tool_params = parsed["arguments"]
            result.text = text  # 保留原文以便调试
